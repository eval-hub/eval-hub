...
func TestLoadConfig(t *testing.T) {
	...
	t.Run("loading config from tests directory", func(t *testing.T) {
		serviceConfig, err := config.LoadConfig(logger, "0.0.2", "local", time.Now().Format(time.RFC3339), "../../tests")
		if err != nil {
...
`
		...
		serviceConfig, err := config.LoadConfig(logger, "0.0.2", "local", time.Now().Format(time.RFC3339), baseDir)
		if err != nil {
			t.Fatalf("Failed to load config: %v", err)
		}
...
`
	...
	t.Run("loading config from secrets directory", func(t *testing.T) {
		...
		serviceConfig, err := config.LoadConfig(logger, "0.0.2", "local", time.Now().Format(time.RFC3339), "../../tests/secrets")
		if err != nil {
			t.Fatalf("Failed to load config: %v", err)
		}
		if serviceConfig == nil {
