...
func (s *SQLiteStorage) CreateCollection(collection *api.CollectionResource) error {
...
func (s *SQLiteStorage) GetCollection(id string) (*api.CollectionResource, error) {
	e := sql.SQLExecutor{
		Db:  s.pool,
		Ctx: s.ctx,
	}
...
func (s *SQLiteStorage) GetCollections(filter abstractions.QueryFilter) (*abstractions.QueryResults[api.CollectionResource], error) {
...
func (s *SQLiteStorage) UpdateCollection(collection *api.CollectionResource) error {
	err := sql.WithTransaction(s.pool, s.ctx, s.logger, "update collection", collection.Resource.ID, func(txn *db.Tx) error {
		...
		return updateCollectionTransactional(collection.Resource.ID, persistedCollection, e)
	})
	return err
}
...
func (s *SQLiteStorage) PatchCollection(id string, patches *api.Patch) error {
	err := sql.WithTransaction(s.pool, s.ctx, s.logger, "patch collection", id, func(txn *db.Tx) error {
		...
		return updateCollectionTransactional(id, persistedCollection, e)
	})
	return err
}
...
func updateCollectionTransactional(collectionID string, collection *api.CollectionResource, e sql.SQLExecutor) error {
	collectionJSON, err := sql.CreateCollectionEntity(collection)
	if err != nil {
		return serviceerrors.NewServiceError(messages.InternalServerError, "Error", err)
	}
	updateCollectionStatement, args := createUpdateCollectionStatement(collectionID, string(collectionJSON))
