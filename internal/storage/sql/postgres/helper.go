...
const INSERT_EVALUATION_STATEMENT = `INSERT INTO evaluations (id, tenant_id, status, experiment_id, entity) VALUES ($1, $2, $3, $4, $5) RETURNING id;`
const INSERT_COLLECTION_STATEMENT = `INSERT INTO collections (id, tenant_id, entity) VALUES ($1, $2, $3) RETURNING id;`
const INSERT_PROVIDER_STATEMENT = `INSERT INTO providers (id, tenant_id, entity) VALUES ($1, $2, $3) RETURNING id;`

const GET_EVALUATION_STATEMENT = `SELECT id, created_at, updated_at, tenant_id, status, experiment_id, entity FROM evaluations WHERE id = $1;`
const GET_COLLECTION_STATEMENT = `SELECT id, created_at, updated_at, tenant_id, entity FROM collections WHERE id = $1;`
const GET_PROVIDER_STATEMENT = `SELECT id, created_at, updated_at, tenant_id, entity FROM providers WHERE id = $1;`
...
const UPDATE_STATEMENT = `UPDATE %s SET %s WHERE id = $%d;`
const CHECK_ENTITY_EXISTS_STATEMENT = `SELECT id FROM %s WHERE id = $1;`
const DELETE_STATEMENT = `DELETE FROM %s WHERE id = $1;`
...
func getParamValue(index int) string {
...
func createFilterStatement(filter map[string]any, orderBy string, limit int, offset int) (string, error) {
	...
	if len(filter) > 0 {
		...
		for key := range filter {
			...
			fmt.Fprintf(&sb, "%s = %s", key, getParamValue(index))
			index++
		}
	}
	...
	if limit > 0 {
		fmt.Fprintf(&sb, " LIMIT %s", getParamValue(index))
		index++
	}
	if offset > 0 {
		fmt.Fprintf(&sb, " OFFSET %s", getParamValue(index))
		index++
	}
...
func createUpdateEvaluationStatement(id string, status api.OverallState, entityJSON string) (string, []any) {
	...
	query := fmt.Sprintf(UPDATE_STATEMENT, "evaluations", set, whereIdx)
	args = append(args, id)
	return query, args
}

func createUpdateCollectionStatement(id string, entityJSON string) (string, []any) {
	query := fmt.Sprintf(UPDATE_STATEMENT, "collections", "entity = $1, updated_at = CURRENT_TIMESTAMP", 2)
	args := []any{entityJSON, id}
	return query, args
}

func createUpdateProviderStatement(id string, entityJSON string) (string, []any) {
	query := fmt.Sprintf(UPDATE_STATEMENT, "providers", "entity = $1, updated_at = CURRENT_TIMESTAMP", 2)
	args := []any{entityJSON, id}
	return query, args
}
