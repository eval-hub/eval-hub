...
func createProviderCountStatement(driver, tableName string, params map[string]any) (string, []any, error) {
	quotedTable := quoteIdentifier(driver, tableName)
	where, args, err := createProviderFilterWhereAndArgs(driver, params)
	if err != nil {
		return "", nil, err
	}
	return fmt.Sprintf("SELECT COUNT(*) FROM %s%s;", quotedTable, where), args, nil
...
func createProviderListStatement(driver, tableName string, limit, offset int, params map[string]any) (string, []any, error) {
	quotedTable := quoteIdentifier(driver, tableName)
	where, args, err := createProviderFilterWhereAndArgs(driver, params)
	if err != nil {
		return "", nil, err
