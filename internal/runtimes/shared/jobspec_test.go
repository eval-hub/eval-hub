...
func TestBuildJobSpecJSONHappyPath(t *testing.T) {
	...
	result, err := BuildJobSpecJSON(eval, "provider-1", "bench-1", &callbackURL)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
...
func TestBuildJobSpecJSONNilCallbackURL(t *testing.T) {
	...
	result, err := BuildJobSpecJSON(eval, "provider-1", "bench-1", nil)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
...
func TestBuildJobSpecJSONNoNumExamples(t *testing.T) {
	...
	result, err := BuildJobSpecJSON(eval, "provider-2", "bench-2", nil)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
...
func TestBuildJobSpecJSONBenchmarkNotFound(t *testing.T) {
	eval := baseEvaluation()
	_, err := BuildJobSpecJSON(eval, "provider-1", "nonexistent", nil)
	if err == nil {
		t.Fatal("expected error, got nil")
	}
	if !strings.Contains(err.Error(), "benchmark config not found") {
...
func TestBuildJobSpecJSONDoesNotMutateOriginalParams(t *testing.T) {
	...
	_, err := BuildJobSpecJSON(eval, "provider-1", "bench-1", nil)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
