...
func TestBuildJobConfigWithOCIOutputs(t *testing.T) {
	t.Setenv(serviceURLEnv, "http://eval-hub")
	evaluation := &api.EvaluationJobResource{
		Resource: api.EvaluationResource{
		...
		EvaluationJobConfig: api.EvaluationJobConfig{
			...
			Outputs: &api.EvaluationOutputs{
				OCI: &api.EvaluationOutputsOCI{
	...
	if !ok {
		t.Fatalf("expected outputs in job spec json, got %v", decoded["outputs"])
	}
	oci, ok := outputs["oci"].(map[string]any)
	if !ok {
		t.Fatalf("expected outputs.oci in job spec json, got %v", outputs["oci"])
	}
	coords, ok := oci["coordinates"].(map[string]any)
	if !ok {
		t.Fatalf("expected outputs.oci.coordinates in job spec json, got %v", oci["coordinates"])
	}
