...
func TestEvaluationJobConfigHardwareConfigRoundTrip(t *testing.T) {
	...
	if cfg.Benchmarks[0].HardwareConfig.HardwareProfileRef.Name != "cpu-optimized-profile" {
		t.Fatalf("name = %q, want cpu-optimized-profile", cfg.Benchmarks[0].HardwareConfig.HardwareProfileRef.Name)
	}
	...
	if _, hasNamespace := profileRef["namespace"]; hasNamespace {
		t.Fatalf("namespace should not be present on hardware_profile_ref, got: %s", string(encoded))
	}
}
