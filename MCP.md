...
| `create_collection` | Required: `name`, `benchmarks`, and one of `category` or `domains`. Optional: `description`, `tags`, `pass_criteria`, plus whichever of `category`/`domains` was not used. | Create a new benchmark collection (`category` is deprecated; prefer `domains`). Use after designing a collection (via the `design_collection` prompt or manually) to persist it. The collection can then be referenced in `submit_evaluation`. |
