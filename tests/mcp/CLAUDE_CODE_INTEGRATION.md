...
| `submit_evaluation` | `name`;<br><br>**`model`**: REST `ModelRef` (`url`, `name`, optional `parameters`, optional `auth.secret_ref`);<br><br>either **`benchmarks`**: REST `EvaluationBenchmarkConfig` list **or** **`collection`**: `CollectionRef` (e.g. `leaderboard-v2`);<br><br>optional: `description`, `tags`, `experiment` | Submit an evaluation job |
