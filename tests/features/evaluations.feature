...
Feature: Evaluations Endpoint
  ...
  Scenario: Owner isolation for evaluation jobs
    ...
    Then the response code should be 200
    And the response should contain the value "iso-user-2" at path "$.items[0].resource.owner"
    # iso-user-1 lists jobs — should see only their own
    And I set the header "X-User" to "iso-user-1"
    When I send a GET request to "/api/v1/evaluations/jobs"
    Then the response code should be 200
    And the response should contain the value "iso-user-1" at path "$.items[0].resource.owner"
  ...
  @local
  Scenario: Update evaluation job status with phase
    Given the service is running
    When I send a POST request to "/api/v1/evaluations/jobs" with body "file:/evaluation_job.json"
    Then the response code should be 202
    When I send a POST request to "/api/v1/evaluations/jobs/{id}/events" with body "file:/evaluation_job_status_event_running_with_phase.json"
    Then the response code should be 204
    When I send a GET request to "/api/v1/evaluations/jobs/{id}"
    Then the response code should be 200
    And the response should contain the value "running" at path "$.status.state"
    And the response should contain the value "running_evaluation" at path "$.status.benchmarks[0].phase"
    When I send a POST request to "/api/v1/evaluations/jobs/{id}/events" with body "file:/evaluation_job_status_event_completed.json"
    Then the response code should be 204
    When I send a DELETE request to "/api/v1/evaluations/jobs/{id}?hard_delete=true"
    Then the response code should be 204
  ...
  @local
  Scenario: Update evaluation job status with invalid phase returns 400
    Given the service is running
    When I send a POST request to "/api/v1/evaluations/jobs" with body "file:/evaluation_job.json"
    Then the response code should be 202
  ...
  # This will no longer work with the logic in the events API
  @ignore
  @local
  ...
  Scenario: List evaluation jobs filtered by status
    ...
    Then the response code should be 200
    # TODO And the response should contain the value "running" at path "$.items[0].status.state"
    # And the response should have schema as:
    #"""
    #  {
    #    "properties": {
  ...
  Scenario: List evaluation jobs with all search filters
    ...
    Then the response code should be 200
    # TODO And the response should contain the value "running" at path "$.items[0].status.state"
    # TODO And the response should contain at least the value "1" at path "$.total_count"
    ...
    Then the response code should be 200
    And the response should equal the value "0" at path "$.total_count"


  # TODO Check filtering by user and then create some tests
  ...
  Scenario: List evaluation jobs with all search filters for multiple users
    ...
    When I send a GET request to "/api/v1/evaluations/jobs"
    Then the response code should be 200
    And the "total_count" field in the response should be saved as "value:total_count_ej1"
