Feature: Providers Endpoint
  ...
  Scenario: Get provider for existent provider id
    ...
    And the response should contain the value "garak" at path "items[0].resource.id"
  ...
  Scenario: Get provider with benchmarks
    ...
    Then the response code should be 200
    Then the response should contain the value "garak" at path "items[0].resource.id"
