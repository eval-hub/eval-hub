...
Feature: Evaluation Jobs
  ...
  "$id": "https://eval-hub.github.io/test/features/test_data/schemas/evaluation_job.schema.json",
  "title": "EvaluationJobConfig",
  "description": "Evaluation job request body (benchmarks variant). Matches pkg/api.EvaluationJobConfig and docs OpenAPI EvaluationJobConfigBenchmarks.",
  "type": "object",
  "required": ["name", "model", "benchmarks"],
  ...
  "properties": {
    ...
    "tags": {
      ...
      "uniqueItems": true,
      "description": "Tags for categorizing the job (no comma or pipe characters)."
    },
    "model": {
      "$ref": "#/$defs/ModelRef"
    },
  ...
  "$defs": {
    "ModelRef": {
      ...
      "properties": {
        "url": {
          ...
          "format": "uri",
          "minLength": 1,
          "description": "Model inference endpoint URL."
        },
        "name": {
          "type": "string",
          "minLength": 1,
    ...
    "ModelAuth": {
      ...
      "properties": {
        "secret_ref": {
          "type": "string",
          "minLength": 1,
          "description": "Kubernetes Secret name for model authentication credentials."
        }
      }
    },
    "EvaluationBenchmarkConfig": {
      ...
      "properties": {
        ...
        "weight": {
          "type": "number",
          "minimum": 0,
          "description": "Relative weight of this benchmark."
        },
        "primary_score": {
          "$ref": "#/$defs/PrimaryScore"
        },
        "pass_criteria": {
          "$ref": "#/$defs/PassCriteria"
        },
        "parameters": {
          ...
          "properties": {
            "tokenizer": {
              "type": "string",
              "const": "google/flan-t5-small",
              "description": "Tokenizer to use for the benchmark."
            }
          },
          "additionalProperties": true,
          "description": "Provider-specific benchmark parameters."
        },
        "test_data_ref": false
      }
    },
    ...
    "PassCriteria": {
      ...
      "properties": {
        "threshold": {
          "type": "number",
          "description": "Pass/fail threshold for the primary score."
        }
      }
    },
    "ExperimentConfig": {
  ...
  "$defs": {
    ...
    "EvaluationBenchmarkConfig": {
      "type": "object",
      "required": ["id", "provider_id", "test_data_ref"],
      "additionalProperties": false,
      "properties": {
        "id": {
          "type": "string",
    ...
    "TestDataRef": {
      ...
      "properties": {
        "s3": {
          "$ref": "#/$defs/S3TestDataRef"
        }
      },
      "description": "External test data for Kubernetes Job runs (init container downloads to /test_data)."
    },
    "S3TestDataRef": {
      "type": "object",
      "required": ["bucket", "key", "secret_ref"],
      "additionalProperties": false,
      "properties": {
        "bucket": {
          "type": "string",
        ...
        "key": {
          "type": "string",
          "minLength": 1,
          "description": "Object key or prefix within the bucket."
        },
        "secret_ref": {
          "type": "string",
          "minLength": 1,
          "description": "Kubernetes Secret name with AWS/S3 credentials."
        }
      }
    },
    "ExperimentConfig": {
