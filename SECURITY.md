...
2. Verify online (queries the GitHub attestation API):
...
3. Or verify offline with the Sigstore bundle from the release:
...
`SHA256SUMS` is an integrity aid for the archive; it is not a Scorecard-recognized signature. The Scorecard-recognized artifact is the `provenance.sigstore.json` bundle.
