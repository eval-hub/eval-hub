...
import (
	...
	"github.com/eval-hub/eval-hub/eval_runtime_sidecar/api/common"
...
func NewRespWrapper(response http.ResponseWriter, ctx *executioncontext.ExecutionContext) http_wrappers.ResponseWrapper {
	return RespWrapper{
		Response: response,
		ctx:      ctx,
	}
...
func (r RespWrapper) errorWithMessageCode(requestId string, messageCode *messages.MessageCode, messageParams ...any) {
	...
	r.WriteJSON(common.Error{Message: msg, MessageCode: messageCode.GetCode(), Trace: requestId}, messageCode.GetStatusCode())
...
func (r RespWrapper) Error(err error, requestId string) {
	if e, ok := err.(common.ServiceError); ok {
